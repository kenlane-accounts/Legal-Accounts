class Tranheadpinvs::PcnomsController < ApplicationController
    before_action :set_tranhead, only: [:edit, :update, :destroy]
  
  def new
    @i=0
    @tranhead = Tranheads::Pinv.new
    @tranhead.trans << Tranpcnom.new
  end
  ...
  def create
    ...
    respond_to do |format|
      ...
      if @tranhead.save
        format.html { redirect_to tranheadpinvs_path, notice: 'Credit Note was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @tranhead.update(tranhead_params)
        format.html { redirect_to tranheadpinvs_path, notice: 'Credit Note was successfully updated.' }
  ...
  private
    ...
    def tranhead_params
      params.require(:tranheads_pinv).permit(:trref, :trotherref, :trdate, :supplier_id, :type, 
        trans_attributes: [:id, :trdetails, :tramount, :nominal_id, :netamount, :vat_id, :vatcode, :vatperc, :vatamount, :type, :_destroy])
