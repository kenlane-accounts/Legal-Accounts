class Tranheadsinvs::ScmatsController < ApplicationController
    before_action :set_tranhead, only: [:edit, :update, :destroy]
  
  def new
    @i=0
    @tranhead = Tranheads::Sinv.new
    @tranhead.trans << Transcmat.new
  end
  ...
  def create
    ...
    respond_to do |format|
      ...
      if @tranhead.save
        format.html { redirect_to tranheadsinvs_path, notice: 'Credit Note was successfully created.' }
  ...
  def edit
     @tranhead.trans.each do |item|
        ...
        if !item.outamount.nil?
          item.outamount=item.outamount*-1
        end
    end 
  end
  ...
  def update
    respond_to do |format|
      if @tranhead.update(tranhead_params)
        format.html { redirect_to tranheadsinvs_path, notice: 'Credit Note was successfully updated.' }
