class Tranheads::OrmatsController < ApplicationController
    before_action :set_tranhead, only: [:edit, :update, :destroy]
  
  def new
    @i=0
    @tranhead = Tranheads::Bank.new
    @tranhead.trans << Tranormat.new
  end
  ...
  def create
    ...
    respond_to do |format|
      ...
      if @tranhead.save
        format.html { redirect_to tranheads_path, notice: 'Receipt was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @tranhead.update(tranhead_params)
        format.html { redirect_to tranheads_path, notice: 'Receipt was successfully updated.' }
  ...
  private
    ...
    def tranhead_params
      params.require(:tranheads_bank).permit(:trref, :trdate, :bank_id, :type, 
        trans_attributes: [:id, :trdetails, :tramount, :case_id, :type, :outlaytype_id, :thirdp, :troutlaybill, :fromclientbank_id, :askclientoverdraw, :clientoverdraw, :_destroy])
