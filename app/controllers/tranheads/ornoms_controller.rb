class Tranheads::OrnomsController < ApplicationController
    before_action :set_tranhead, only: [:edit, :update, :destroy]
  
  def new
    @i=0
    @tranhead = Tranheads::Bank.new
    @tranhead.trans << Tranornom.new
  end
  ...
  private
    ...
    def tranhead_params
      params.require(:tranheads_bank).permit(:trref, :trdate, :bank_id, :type, 
        trans_attributes: [:trdetails, :tramount, :id, :nominal_id, 
            :vat_id, :vatperc, :vatamount, :netamount, :type, :_destroy])
