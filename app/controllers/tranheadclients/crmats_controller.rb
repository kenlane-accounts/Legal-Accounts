class Tranheadclients::CrmatsController < ApplicationController
    before_action :set_tranhead, only: [:edit, :update, :destroy]
  
  def new
    @i=0
    @tranhead = Tranheads::Bankclient.new
    @tranhead.trans << Trancrmat.new
  end
  ...
  def create
    ...
    respond_to do |format|
      ...
      if @tranhead.save
        format.html { redirect_to tranheadclients_path, notice: 'Client receipt was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @tranhead.update(tranhead_params)
        format.html { redirect_to tranheadclients_path, notice: 'Client receipt was successfully updated.' }
  ...
  private
    ...
    def tranhead_params
      params.require(:tranheads_bankclient).permit(:trref, :trdate, :bank_id, :trpayrec, :type, 
